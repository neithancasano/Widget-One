...
RejectEmojis = ["❌"]
TierLevels = [
  "🪵 Woodcutter", "🛖 Carpenter", "🪨 Stonemason", "🧱 Bricklayer", "🏠 Architect",
  "🏛 Philosopher", "🔩 Engineer", "🚀 Astronaut", "🪐 Stargazer"
]
TierLevelsEmojiOnly = ["🪵", "🛖", "🪨", "🧱", "🏠", "🏛", "🔩", "🚀", "🪐"]
TierReqsPerLevel = [
  [],  #level 1
  ...
  if str(user) in GameMasters and str(reaction) in AcceptEmojis:
    ...
    if str(message.author.id) in data:
      ...
      try:
        ...
        if int(data[str(
            message.author.id)][message.channel.name]["tierXP"]) >= int(
              data[str(message.author.id)][message.channel.name]
              ["tierLVL"])**tierLVLexponent + tierLVLoffset:
          ...
          embed = discord.Embed(
            ...
            description=
            f"{message.author.name}\'s {message.channel.name} proficiency has reached the [{TierLevels[int(data[str(message.author.id)][message.channel.name]['tierLVL'])-1]}] Tier",
            color=0xFFA500)
      ...
      for i in data[str(message.author.id)]:
        if i in channelnames:
          await message.channel.send(
            f"{i} {data[str(message.author.id)][i]['tierLVL']}")
      ...
      await message.channel.send(
        f"req to unlock next tier is: {TierReqsPerLevel[int(curr_LVL)]}\n" +
      ...
      if missing_emojis:
        await message.channel.send(
          f"You are missing the following: {missing_emojis}")
        RequirementsSatisfied = False
      else:
      ...
      acceptEmbed = discord.Embed(
        title=f"Solution/Proof Accepted! ✅",
      ...
      await message.channel.send(
        f"can't find: {message.author.id} in player database\nPlease contact Game Masters"
      )

    with open("levels.json", "w") as f:
      json.dump(data, f)
  elif str(user) in GameMasters and str(reaction) in RejectEmojis:
    rejectEmbed = discord.Embed(
      title=f"Solution/Proof not Accepted! ❌",
    ...
    if message.content.startswith("$stats"):
      ...
      try:
        ...
        self_xpuntilnextlevel = round(
          int(data[str(message.author.id)]["LVL"])**LVLexponent +
          LVLoffset) - data[str(message.author.id)]["XP"]
        ...
        if curr_LVL < MaxLevel:
          ...
          await message.channel.send(
            f"Max level reached no next tier to unlock")
        ...
        if missing_emojis:
          ...
          await message.channel.send(
            "Tier requirements for level up sufficient.")
          RequirementsSatisfied = True
        ...
        for i in data[str(message.author.id)]:
          if i == "LVL":
            ...
            tXPd = round(
              int(data[str(message.author.id)][i]["tierLVL"])**tierLVLexponent
              + tierLVLoffset)
        ...
        noStats = discord.Embed(
          title=message.author.name + " ⚔️",
